...
class Checkbox extends \yii\base\Widget
{
    ...
    public function run()
    {
        ...
        // $new[] = Html::input('text', 'variant_value', '', [
            ...
            // 'data-create-action' => Url::toRoute(['/filter/filter-variant/create']), 
            // 'class' => ' form-control'
        // ]);
        // $new[] = Html::button(Html::tag('i', '', [
    ...
    private function variant($item)
    {
        ...
        $return[] = ' ';
        $return[] = Html::label(json_decode($item->value)->{Yii::$app->language}, 'filtervariant'.$item->id);
